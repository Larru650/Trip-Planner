...
namespace TheWorld.Models
{
    public class WorldContextSeedData
    {
        ...
        public WorldContextSeedData(WorldContext context, UserManager<WorldUser> userManager) //dependency injection injected into the constructor
        {
            _context = context;
            _userManager = userManager; //the user manager will allow me to add, remove and change users.
        }

        public async Task EnsureSeedData() //one of th calls is asynchronous so we'll make this async
        {
            if (await _userManager.FindByEmailAsync("jordicarulla@theworld.com") == null)
            {
                var user = new WorldUser()
                {
                    UserName = "jordicarulla",
                    Email = "jordicarulla@theworld.com"
                };
            ...
            if (!_context.Trips.Any())
            {
                var usTrip = new Trip()
                {
                    ...
                    Name = "US Trip",
                    UserName = "jordicarulla", //TODO add Username
                    Stops = new List<Stop>()
                    {
                ...
                _context.Stops.AddRange(usTrip.Stops); //as stops is a collection, we need to use AddRange and not just Add
            ...
            new Stop() { Order = 55, Latitude =  33.748995, Longitude =  -84.387982, Name = "Atlanta, Georgia", Arrival = DateTime.Parse("Jun 17, 2015") },
                ...
                await _context.SaveChangesAsync(); //this will go and push this info to the database and save. Await has to be used for async methods
